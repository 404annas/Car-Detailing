      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-8">
            <div className="max-w-2xl">
                <span className="text-green-500 font-bold uppercase text-xs mb-4 block orb">
            ...
            <p className="text-gray-400 max-w-sm text-sm leading-normal text-right md:text-left hidden md:block border-l border-green-800 pl-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {steps.map((step, index) => (
            <div 
                key={index} 
                className="group relative bg-[#121826] border border-green-800 hover:border-green-500/50 rounded-xl p-8 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-green-900/10 flex flex-col min-h-[420px]"
            >
                {/* Large Watermark Number */}
                <span className="absolute top-2 right-6 text-[140px] font-black text-gray-800/10 group-hover:text-green-500/5 transition-colors duration-500 select-none orb leading-none z-0">
                    {step.id}
                </span>
                ...
                <div className="relative z-10 h-full flex flex-col">
                    ...
                    {/* Icon Box */}
                    <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white group-hover:bg-green-500 group-hover:text-black group-hover:border-green-500 transition-all duration-300 shadow-lg mb-8">
                        {step.icon}
                    </div>

                    {/* Titles */}
                    <h4 className="text-green-500 font-bold text-xs uppercase mb-2 orb">
                        {step.subtitle}
                    </h4>
                    ...
                    {/* Description */}
                    <p className="text-gray-400 leading-normal mb-8 border-t border-green-800 pt-6 group-hover:border-green-700 transition-colors">
                        {step.desc}
                    </p>
                    ...
                    <div className="mt-auto flex flex-wrap gap-2">
                        {step.tags.map((tag, i) => (
                            <span key={i} className="text-[10px] font-bold uppercase tracking-wide bg-gray-900 text-gray-400 px-3 py-1.5 rounded-full border border-green-700 group-hover:border-green-500/30 group-hover:text-green-500 transition-colors">
                                {tag}
                            </span>
                        ))}
                    </div>
